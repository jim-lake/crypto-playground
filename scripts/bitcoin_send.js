...
const hdnode = ethers.utils.HDNode.fromMnemonic(config.SIGN_MNEMONIC).derivePath(path);
...
const signer = ecpair.ECPair.fromPrivateKey(Buffer.from(ethers.utils.arrayify(hdnode.privateKey)));

makeTransaction();

async function makeTransaction() {
  const { high_fee_per_kb, medium_fee_per_kb, low_fee_per_kb } = await getFeeRate();
  console.error("fee(high, medium, low):", high_fee_per_kb, medium_fee_per_kb, low_fee_per_kb);
  const fee_per_byte = Math.ceil(argv[3] ? parseInt(argv[3]) : high_fee_per_kb / 1024);
  ...
  let bounce_count = 0;
  while (1) {
    const result = _incrementalTransaction(tx_list, coins, fee_per_byte, target_fee);
