...
contract Moloch {
  ...
  constructor(
    ...
    require(
      _emergencyExitWait > 0,
      'Moloch::constructor - _emergencyExitWait cannot be 0'
    );
    require(
      _dilutionBound > 0,
    ...
    require(
      _approvedTokens.length > 0,
      'Moloch::constructor - need at least one approved token'
    );
    require(
    ...
    for (uint256 i = 0; i < _approvedTokens.length; i++) {
      ...
      require(
        ...
        'Moloch::constructor - duplicate approved token'
      );
  ...
  function submitProposal(
    ...
    require(
      ...
      'Moloch::submitProposal - tributeToken is not whitelisted'
    );
    require(
      tokenWhitelist[paymentToken],
      'Moloch::submitProposal - payment is not whitelisted'
    );
    require(
      applicant != address(0),
  ...
  function submitWhitelistProposal(
    ...
    require(
      ...
      'Moloch::submitWhitelistProposal - must provide token address'
    );
    require(
      !tokenWhitelist[tokenToWhitelist],
      "Moloch::submitWhitelistProposal - can't already have whitelisted the token"
    );
  ...
  {
    require(
      members[memberToKick].shares > 0,
      'Moloch::submitGuildKickProposal - member must have at least one share'
    );
  ...
  function sponsorProposal(uint256 proposalId) public onlyDelegate {
    require(
      depositToken.transferFrom(msg.sender, address(this), proposalDeposit),
    ...
    require(
      !proposal.flags[0],
      'Moloch::sponsorProposal - proposal has already been sponsored'
    );
    require(
      !proposal.flags[3],
      'Moloch::sponsorProposal - proposal has been cancelled'
    );
    ...
    if (proposal.flags[4]) {
      ...
      require(
        totalShares.add(totalSharesRequested).add(proposal.sharesRequested) <=
    ...
    onlyDelegate
  {
    ...
    require(
      proposal.flags[0],
      'Moloch::submitVote - proposal has not been sponsored'
    );
    require(
  ...
  function processProposal(uint256 proposalIndex) public {
    ...
    if (
      ...
      proposal
      .startingPeriod
      .add(votingPeriodLength)
      .add(gracePeriodLength)
      .add(emergencyExitWait)
    ) {
    ...
    if (didPass) {
      ...
      if (proposal.flags[4]) {
        ...
        require(
          proposal.tributeToken.transfer(
            address(guildBank),
  ...
  function safeRagequit(uint256 sharesToBurn, IERC20[] memory tokenList)
    public
    onlyMember
  {
    for (uint256 i = 0; i < tokenList.length; i++) {
      require(
        tokenWhitelist[address(tokenList[i])],
        'Moloch::safeRequit - token must be whitelisted'
      );

      if (i > 0) {
        require(
          tokenList[i] > tokenList[i - 1],
          'Moloch::safeRagequit - tokenList must be unique and in ascending order'
        );
      }
    }
  ...
  function _ragequit(uint256 sharesToBurn, IERC20[] memory approvedTokens)
    internal
  {
  ...
  function cancelProposal(uint256 proposalId) public {
    ...
    require(
      !proposal.flags[0],
      'Moloch::cancelProposal - proposal has already been sponsored'
    );
    require(
      msg.sender == proposal.proposer,
      'Moloch::cancelProposal - only the proposer can cancel'
    );
