...
contract HarbourDeposit is IERC677Receiver, AdminRole, CreatorWithdraw {
  ...
  function _createDeposit(
    ...
    _deposits[depositId] = UserDeposit(sender, coin, block.number, value);
  ...
  function onTokenTransfer(
    ...
    _createDeposit(sender, _msgSender(), value);
  }

  function deposit(address coin, uint256 value) public returns (uint256) {
    bool success = IERC20(coin).transferFrom(
      _msgSender(),
      address(this),
      value
    );
  ...
  function refund(
    ...
    IERC20(coin).transfer(sender, value);
  ...
  function consume(
    ...
    _removeDeposit(depositId, sender, coin, value);
    IERC20(coin).transfer(_msgSender(), value);
