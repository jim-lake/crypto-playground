...
abstract contract BridgeOperationsStorage is EternalStorage {
  ...
  function setMessageToken(bytes32 _messageId, address _token) internal {
    addressStorage[
      keccak256(abi.encodePacked('messageToken', _messageId))
    ] = _token;
  }
  ...
  function messageToken(bytes32 _messageId) internal view returns (address) {
    return
      addressStorage[keccak256(abi.encodePacked('messageToken', _messageId))];
  }
  ...
  function setMessageValue(bytes32 _messageId, uint256 _value) internal {
    uintStorage[
      keccak256(abi.encodePacked('messageValue', _messageId))
    ] = _value;
  }
   ...
   */
  function setMessageRecipient(bytes32 _messageId, address _recipient)
    internal
  {
    addressStorage[
      keccak256(abi.encodePacked('messageRecipient', _messageId))
    ] = _recipient;
  }
   ...
   */
  function messageRecipient(bytes32 _messageId)
    internal
    view
    returns (address)
  {
    ...
    require(
      !isProxy || // covers usage without calling through storage proxy
        (returnData.length == 32 &&
          msg.sender == abi.decode(returnData, (address))) || // covers usage through regular proxy calls
 ...
 */
abstract contract FailedMessagesProcessor is
  BasicAMBMediator,
...
abstract contract NFTBridgeLimits is Ownable {
    ...
    returns (uint256)
  {
    return
      uintStorage[
        keccak256(abi.encodePacked('totalSpentPerDay', _token, _day))
      ];
  }
   ...
   */
  function totalExecutedPerDay(address _token, uint256 _day)
    public
    view
    returns (uint256)
  {
    return
      uintStorage[
        keccak256(abi.encodePacked('totalExecutedPerDay', _token, _day))
      ];
  }
  ...
  function executionDailyLimit(address _token) public view returns (uint256) {
    return
      uintStorage[keccak256(abi.encodePacked('executionDailyLimit', _token))];
  }
  ...
  function withinLimit(address _token) public view returns (bool) {
    return
      ...
      dailyLimit(_token) > totalSpentPerDay(_token, getCurrentDay());
  }
  ...
  function withinExecutionLimit(address _token) public view returns (bool) {
    return
      ...
      executionDailyLimit(_token) >
      totalExecutedPerDay(_token, getCurrentDay());
  }
  ...
  function addTotalSpentPerDay(address _token) internal {
    uintStorage[
      keccak256(abi.encodePacked('totalSpentPerDay', _token, getCurrentDay()))
    ] += 1;
  }
  ...
  function addTotalExecutedPerDay(address _token) internal {
    uintStorage[
      keccak256(
        abi.encodePacked('totalExecutedPerDay', _token, getCurrentDay())
      )
    ] += 1;
  }
  ...
  function _setDailyLimit(address _token, uint256 _dailyLimit) internal {
    uintStorage[
      keccak256(abi.encodePacked('dailyLimit', _token))
    ] = _dailyLimit;
   ...
   */
  function _setExecutionDailyLimit(address _token, uint256 _dailyLimit)
    internal
  {
    uintStorage[
      keccak256(abi.encodePacked('executionDailyLimit', _token))
    ] = _dailyLimit;
...
contract ReentrancyGuard {
  function lock() internal view returns (bool res) {
    assembly {
      ...
      res := sload(
        0x6168652c307c1e813ca11cfb3a601f1cf3b22452021a5052d8b05f1f1f8a3e92
...
contract NativeTokensRegistry is EternalStorage {
   ...
   */
  function isRegisteredAsNativeToken(address _token)
    public
    view
    returns (bool)
  {
   ...
   * @return message id of the send message.
   */
  function tokenRegistrationMessageId(address _token)
    public
    view
    returns (bytes32)
  {
    return
      bytes32(
        uintStorage[
          keccak256(abi.encodePacked('tokenRegistrationMessageId', _token))
        ]
      );
  }
   ...
   */
  function _setTokenRegistrationMessageId(address _token, bytes32 _messageId)
    internal
  {
    uintStorage[
      keccak256(abi.encodePacked('tokenRegistrationMessageId', _token))
...
contract ERC721Reader is Ownable {
  ...
  function setCustomMetadata(
    ...
    stringStorage[
      keccak256(abi.encodePacked('customSymbol', _token))
    ] = _symbol;
  }
  ...
  function _readName(address _token) internal view returns (string memory) {
    ...
    return
      status
        ...
        : stringStorage[keccak256(abi.encodePacked('customName', _token))];
  }
  ...
  function _readSymbol(address _token) internal view returns (string memory) {
    ...
    return
      status
        ? abi.decode(data, (string))
        : stringStorage[keccak256(abi.encodePacked('customSymbol', _token))];
  }
   ...
   */
  function _readTokenURI(address _token, uint256 _tokenId)
    internal
    view
...
contract TokenImageStorage is Ownable {
  bytes32 internal constant TOKEN_IMAGE_CONTRACT =
  ...
  function setImplementation(address _implementation) external {
    require(
      msg.sender == bridgeContract ||
        msg.sender == IOwnable(bridgeContract).owner()
    );
...
contract NFTMediatorBalanceStorage is EternalStorage {
   ...
   */
  function mediatorOwns(address _token, uint256 _tokenId)
    public
    view
    returns (bool)
  {
    return
      boolStorage[
        keccak256(abi.encodePacked('mediatorOwns', _token, _tokenId))
      ];
  }
  ...
  FailedMessagesProcessor
{
  ...
  {
    return
      isRegisteredAsNativeToken(_token) ||
      nativeTokenAddress(_token) != address(0);
  }
  ...
  function fixMediatorBalance(
    ...
    require(
      _receiver != address(0) && _receiver != mediatorContractOnOtherSide()
    );
    require(isRegisteredAsNativeToken(_token));
  ...
  function _transformName(string memory _name)
    internal
    pure
    virtual
    returns (string memory);
...
// File: contracts/upgradeable_contracts/omnibridge_nft/HomeNFTOmnibridge.sol
